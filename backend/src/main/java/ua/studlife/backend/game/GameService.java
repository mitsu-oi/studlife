...
public class GameService {
    ...
    // ⚠️ ТУТ БУЛО MAX_DAY_JUMP = 1 — «день не може зрушити більше ніж на один
    // за одне збереження». Прибрано свідомо, бо воно ЛАМАЛО ЧЕСНУ ГРУ.
    //
    // Що ставалось: гра навмисно грається без акаунта (офлайн-режим —
    // залізне правило проєкту). Людина проходить 4 дні гостем, потім
    // реєструється — і перший же сейв прилітає з «день 4», коли на сервері
    // щойно створена гра з днем 1. Стрибок на 3 → відмова. І далі НАЗАВЖДИ:
    // сервер прийме лише день 2, якого вже ніколи не буде. Прогрес не
    // синхронізується взагалі, мовчки.
    //
    // Чому не страшно прибрати: стрибок уперед у принципі неможливо
    // відрізнити від чесної гри офлайн. А ось рух НАЗАД (нижче) —
    // це справді спроба переграти невдалий день, і він лишається забороненим.
    //
    // Справжній захист від накруток знадобиться на Етапі 19 (рейтинг), і це
    // буде не перевірка різниці днів, а перевірка всього забігу цілком.
