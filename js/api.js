...
// ⚠️ БУДИМО НАЙПЕРШЕ, ЩЕ ДО ЗАПУСКУ ГРИ.
//
// Цей рядок виконується в ту саму мить, коли браузер прочитав api.js —
// тобто раніше, ніж намалюється кімната й з'явиться будь-яке вікно.
// Поки завантажуються решта файлів, спрайти й фон, сервер уже встає.
//
// Різниця відчутна: пробудження триває до хвилини, і кожна секунда, яку
// ми відіграємо на завантаженні гри, — це секунда, якої гравець не чекає.
apiWake();
...
function apiSyncRun(run) {
  if (!run) {
    // ⚠️ В АКАУНТА ГРИ НЕМА (щойно зареєструвався або почав спочатку).
    //
    // Тоді те, що лежить у браузері, — точно НЕ його гра. Якщо це залишити,
    // на стартовому екрані з'явиться «Продовжити» з чужим днем, гравець
    // натисне його — і пропустить вступ та знайомство з сусідами.
    // Саме на це наступила Даша після реєстрації нового акаунта.
    //
    // Свій сейв (з підписом цього ж логіна) не чіпаємо: він міг з'явитись
    // раніше і просто ще не доїхав на сервер.
    const mineToo = apiLoggedIn() && apiSaveOwner() === apiState.user.username;
    if (!mineToo) apiForgetLocalSave();
    return;
  }
...
async function apiBoot() {
  ...
  if (apiState.reachable === false) {
    // ⚠️ Гравцю — по-людськи, без «сервер недоступний». Він однаково нічого
    // з цим не зробить, а от злякатись за свій прогрес може.
    showToast('📴 Зв\'язку нема — граємо як є, прогрес зачекає тут');
    showStartScreen();
    return;
  }
