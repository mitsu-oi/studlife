...
function aiTakeCard(phase) {
  ...
  // ⚠️ СПЕРШУ ВИКИДАЄМО ПРОСТРОЧЕНЕ — і це виправлення справжнього бага.
  //
  // Було так: картка замовляється наперед, під конкретну фазу. Якщо
  // монетка (AI_SHARE) вирішувала «цю фазу веде рукописна», замовлена
  // картка лишалась у запасі. Наступної фази вона вже НЕ ПІДХОДИЛА —
  // і її не брали. Але й не викидали: вона лежала далі й затуляла собою
  // чергу, а aiPrefetch() бачив «у запасі щось є» і нічого не замовляв.
  //
  // Наслідок: після кожної відмови монетки ШІ замовкав до наступного дня,
  // коли та сама фаза приходила знову. Ззовні це виглядало точно так, як
  // помітила Даша: карток від ШІ помітно менше, ніж каже AI_SHARE.
  //
  // Тепер прострочене просто викидається, і місце звільняється одразу.
  aiState.queue = aiState.queue.filter(c => !c.forPhase || c.forPhase === phase);

  const card = aiState.queue.shift() || null;
...
// Чи лежить у запасі готова картка САМЕ для цієї фази.
//
// Потрібно, щоб не витрачати «спробу» намарно: раніше монетка кидалась
// наосліп, і коли вона казала «беремо ШІ», а картка ще не встигла прийти —
// фаза все одно діставалась рукописній. Тепер спершу дивимось, чи є що
// брати, і аж тоді вирішуємо.
function aiHasCardFor(phase) {
  return aiAvailable()
      && aiState.queue.some(c => !c.forPhase || c.forPhase === phase);
}
