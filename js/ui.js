...
/**
 * ЧИ ПОКАЗУВАТИ КАРТКУ ОКРЕМИМ ВІКНОМ (а не в зоні внизу).
 *
 * ⚠️ Поріг ТУТ ШИРШИЙ, ніж «телефон» (900 проти 760), і це не помилка.
 *
 * Причина: у вузькому вікні три варіанти вибору перестають вміщатись в один
 * ряд і стають стовпчиком. Три кнопки заввишки — це вже вся зона, і текст
 * ситуації стискається до нуля. Гравець бачить кнопки й не бачить питання,
 * на яке відповідає.
 *
 * Тому від 900 пікселів і вужче поводимось як на телефоні: внизу лишається
 * короткий натяк і одна кнопка, а сама картка відкривається вікном, де
 * місця вдосталь.
 */
...
function renderDecisionZone() {
  ...
  if (useCardModal() && card && !isNight && cardState.resolvedText === null) {
    zone.innerHTML = `
...
function showAuthScreen(mode = 'login', message = '') {
  ...
  async function submit() {
    ...
    // Сервер може прокидатись зі сну до хвилини, а з повторною спробою —
    // ще довше. Тому кнопка одразу каже, скільки це може тривати: інакше
    // здається, що гра зависла, і людина тисне ще раз або йде геть.
    goBtn.disabled = true;
    goBtn.textContent = '🪳 Будимо сервер… (до хвилини)';
...
const INTRO_SLIDES = [
  {
    ...
    body: 'Львів. Політех. Гуртожиток №11 — та сама «Віп-общага».<br>' +
          'Вид на студмістечко, таргани в комплекті.<br><br>' +
          '30 днів до стипендії. Погнали.',
  },
  {
    // Блочна система — це не дрібниця, а половина побуту в грі: душ і туалет
    // спільні на дві кімнати, тому вони й ламаються «на всіх» (див.
    // block_shower / block_lights / block_door у data/force_majeure.js).
    emoji: '🚪', title: 'Твій блок',
    body: 'Общага блочна: <b>дві кімнати мають спільний передпокій, ' +
          'душ і туалет</b>. Разом це блок на вісьмох — по четверо в кімнаті.<br><br>' +
          'У другій кімнаті — сусіди по блоку. Життя спільне: черга в душ, ' +
          'спільна темрява, коли вибиває світло, і спільні розмови в передпокої.<br><br>' +
          '🚿 <b>І головне правило:</b> гаряча вода є не завжди, а ' +
          '<b>з 6:00 до 10:00 і з 18:00 до 23:00</b>. Не встиг у вікно — ' +
          'або крижана, або чекай наступного разу.',
  },
  {
    char: 'player',
  ...
  {
    // ⚠️ Степан НАВМИСНО без char: спрайта в нього нема, бо його самого
    // нема в кімнаті. У кімнаті стоїть його порожнє ліжко — четверте місце
    // рахується за ним, але живе він на квартирі. Присутній лише в чаті
    // «11 садиба» і зрідка — запрошенням кудись вибратись.
    emoji: '📱', title: 'І Степан. Десь.',
    body: 'Четверте місце в кімнаті рахується за Степаном. Ліжко зникло як і сам Степан, ' +
          'тумбочка вже занята речами тусовщика.<br><br>' +
          'Він винаймає квартиру, а в общазі числиться «для порядку». ' +
          'Зате живе в чаті «11 садиба»: коментує все, знає все, ' +
          'і раз на кілька тижнів пише «хлопці, є тема, збираємось?».<br><br>' +
          '<i>Вам, до речі, пощастило: замість чотирьох у кімнаті вас троє.</i>',
  },
  {
