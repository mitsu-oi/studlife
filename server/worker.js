...
function pickTopic(recentTopics = [], place = 'home') {
  const fitting = TOPICS.filter((t) => !t.where || t.where.includes(place));
...
export default {
  async fetch(request, env) {
    // Браузер спершу питає «а мені можна?» — відповідаємо «так»
    ...
    if (request.method === 'GET') {
      ...
      if (url.pathname === '/models') {
        try {
          ...
          return json({ current: MODEL, available });
        } catch (error) {
          return json({ error: String(error.message || error) }, 500);
        }
      }

      return json({ ok: true, message: 'Поштова скринька StudLife працює 🎮', model: MODEL });
    }

    if (request.method !== 'POST') {
      return json({ error: 'Сюди треба надсилати POST' }, 405);
    }

    try {
      const state = await request.json();
      const { prompt, topicKey, morning } = buildPrompt(state);
      const { card: raw, model: usedModel } = await callLLM(prompt, env, state.model, state.thinking);
      return json({
        card: sanitizeCard(raw, topicKey, morning),
